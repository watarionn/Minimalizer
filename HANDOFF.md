...
The user-supplied white-hair case activates the three-slab route; the red-poster case activates the stricter two-slab-poster route. Both now preserve explicit face/hair masses instead of collapsing into giant anonymous polygons. Fixed 16-image evaluation shows **0/16 Phase 12 activations**, preserving the established Phase 11 corpus output profile. Web metadata is moved to candidate v0.10.0 / Rinka phase12, but production remains v0.9.0 until explicit merge/deploy approval. GitHub Actions must remain skipped while metered usage is unsafe; validate locally and create only a Draft PR at closure. See docs/RINKA_PHASE12_HEAD_BODY_ANCHOR_GUARD.md. Local closure on 2026-09-11 passes **265 tests / 2 known missing-fixture deselections**, dedicated Phase 12 **6/6**, compileall, Web JavaScript syntax, `git diff --check`, fixed 16-image evaluation, and real-Uvicorn HTTP 200 smoke for Standard, both Rinka presets, and Color Strip.
