...
### Active Rinka work: Phase 12 Head / Body Anchor Guard

Active branch: eature/rinka-phase12-head-body-anchor-guard-20260910, based on production-verified Phase 11 main at 3483154ccb4c98ca006cc5bc1f3482b99a3bdcee (PR #23). Phase 11 is already merged and deployed.

Phase 12 is a narrow regression rescue for opaque portrait images that are rejected by the normal AI-free segmentation confidence gate and then collapse into two or three giant output slabs. Candidate masks remain disabled for ordinary callers. Rinka Reference may activate them only behind _phase12_portrait_collapse_gate(). The accepted repair rebuilds subject planes, adds a faceless face slab and a large hair anchor, requires torso structure, and passes _phase12_repair_quality_gate() before replacing the baseline. Current quality limits are subject coverage >=0.64 and outside-subject ratio <=0.04. Any failed repair falls back to the pre-Phase-12 result.

The user-supplied white-hair case activates the three-slab route; the red-poster case activates the stricter two-slab-poster route. Both now preserve explicit face/hair masses instead of collapsing into giant anonymous polygons. Fixed 16-image evaluation shows **0/16 Phase 12 activations**, preserving the established Phase 11 corpus output profile. Web metadata is moved to candidate v0.10.0 / Rinka `phase12`, but production remains v0.9.0 until explicit merge/deploy approval. Draft PR **#24** contains the closure candidate and must remain Draft until the user explicitly approves Ready-for-review. GitHub Actions must remain skipped while metered usage is unsafe; validate locally and create only a Draft PR at closure. See `docs/RINKA_PHASE12_HEAD_BODY_ANCHOR_GUARD.md`. Local closure on 2026-09-11 passes **265 tests / 2 known missing-fixture deselections**, dedicated Phase 12 **6/6**, compileall, Web JavaScript syntax, `git diff --check`, fixed 16-image evaluation, and real-Uvicorn HTTP 200 smoke for Standard, both Rinka presets, and Color Strip.
