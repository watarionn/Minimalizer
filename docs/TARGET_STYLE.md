...
Checkpoint 1 makes faceless behavior explicit in post-processing, turns every recognized hand into one six-vertex beveled polygon per known side, and removes a narrow class of decorative crumbs only after identity-bearing masses exist. This is target-style-only behavior; the stable Minimalizer path is unchanged. See `RINKA_PHASE11_GEOMETRIC_POSTER_ABSTRACTION.md`.
