...
Checkpoint 3 adds two explicit Rinka presets without rewriting the accepted subject abstraction. `geometric_poster` is the default and adds three large five-vertex background panels only when subject evidence exists; `faceless_subject` keeps the same Phase 11 subject without synthetic poster panels. The background replacement gate is intentionally stricter than the older generic background classifier, so ordinary midground structure is not deleted merely because it is background-like for cleanup scoring. On the fixed 16-image corpus, poster backgrounds activate on 15 character cases, Night River stays unchanged, all 45 generated panels survive the final cap, and non-poster subject-shape signatures match between the two presets on 16/16 images. The Web v0.9.0 candidate exposes these presets only inside Rinka Reference.


## Phase 12 implementation note: Head / Body Anchor Guard

Phase 12 adds a failure-gated repair path for opaque portrait inputs where the established Rinka pipeline still collapses the subject into two or three dominant slabs. A confidence-rejected AI-free segmentation mask may be retained as a candidate, but it is never activated solely because confidence is low; the output itself must also match a narrow portrait-collapse signature.

An accepted repair must preserve semantic macro structure rather than merely add detail back. It synthesizes one faceless skin-color face slab, one large hair anchor, and requires torso-bearing subject planes. Repairs are accepted only when subject-plane coverage is at least 0.64 and outside-subject overdraw is at most 0.04. Any missing face/hair/torso anchor or failed coverage gate restores the prior baseline output. The fixed 16-image corpus currently triggers this rescue 0/16 times, while the two supplied collapse regressions activate their intended two-slab or three-slab routes. See RINKA_PHASE12_HEAD_BODY_ANCHOR_GUARD.md.

## Phase 13 implementation note: source-color anchors

Phase 13 keeps the Phase 12 faceless geometry intact but improves the source-color choice for face and hair anchors. Face color comes from source-derived skin clusters rather than a broad median; hair color comes from source-derived bright, low-chroma clusters that stay separated from both background and face. The guard may prefer a less-dominant but still real source hair cluster when the dominant cluster is too skin-like. No facial features, hair strands, or arbitrary synthetic recoloring are introduced. See `RINKA_PHASE13_COLOR_ANCHOR.md`.
