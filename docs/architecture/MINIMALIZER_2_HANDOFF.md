...
Current engineering commit: `84fc96e6c324f5511a7fcaa1716f16bd15597f99`
Current branch HEAD: verify the remote branch; review/handoff metadata may be later documentation-only commits
Engineering commit message: `Define Calibration 05 alpha background compatibility`
...
39. This file: `docs/architecture/MINIMALIZER_2_HANDOFF.md`
...
Latest V2 regression after Calibration 05 Phase R: `142 passed`.
Latest full repository regression after Calibration 05 Phase R: `478 passed, 2 failed, 1 warning`.
...
Latest Phase R migration-readiness state: `ready_for_default=false`, `2` blockers; focused alpha/background + migration tests `6 passed`. Fully opaque RGB/RGBA V2 PNG bytes and hashes match exactly.
...
- Phase L: hull-only Region Merge geometry-cost evaluation, targeted Region Merge about `24.33%` faster
- Phase L whole-pipeline same-machine wall time: `5.898851 s -> 5.620624 s` (`4.72%` faster)
- Phase M: pairwise palette-distance reuse, targeted palette about `27.20%` faster
...
Phase P completed the public-control compatibility mapping. Phase Q resolved `output_format_parity`, and Phase R resolved `alpha_background_parity` through conservative compatibility routing without changing opaque canonical pixels. Phase S should define the browser migration and rollback contract while preserving specialized modes and the current production default until explicitly approved.
