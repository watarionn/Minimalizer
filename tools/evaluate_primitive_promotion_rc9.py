...
out=root/'examples/primitive_promotion_rc9'
...
for p in files:
    ...
    off=minimalize(p, replace(base, cleanup_promote_primitives=False))
    on=minimalize(p, base)
    qo,qn=off.metadata.get('quality',{}),on.metadata.get('quality',{})
    cr=on.metadata.get('global_shape_cleanup',{})
    rect=sum(1 for d in cr.get('decisions',[]) if d.get('reason')=='polygon_to_rectangle' and d.get('action')=='promote')
    ell=sum(1 for d in cr.get('decisions',[]) if d.get('reason')=='polygon_to_ellipse' and d.get('action')=='promote')
    vb=cr.get('vertices_before'); va=cr.get('vertices_after')
    row={
      'file':p.name,'subject':bool(on.metadata.get('subject_mode')),
      'shapes_off':len(off.shapes),'shapes_on':len(on.shapes),'promoted_rectangles':rect,'promoted_ellipses':ell,
      'vertices_before_cleanup':vb,'vertices_after_cleanup':va,
...
summary={
 'images':len(rows),'changed_images':sum((r['promoted_rectangles']+r['promoted_ellipses'])>0 for r in rows),
 'promoted_rectangles':sum(r['promoted_rectangles'] for r in rows),'promoted_ellipses':sum(r['promoted_ellipses'] for r in rows),
