...
out=root/'examples/global_shape_value_rc5'
...
for p in files:
    ...
    off=minimalize(p, replace(base, enable_global_shape_value=False))
    on=minimalize(p, replace(base, enable_global_shape_value=True))
    ...
    row={
      ...
      'shapes_off':len(off.shapes),'shapes_on':len(on.shapes),'removed':vr.get('removed_count',0),
...
summary={
 'images':len(rows),'changed_images':sum(r['removed']>0 for r in rows),'removed_shapes':sum(r['removed'] for r in rows),
 ...
 'mean_shape_value':mean('mean_value'),
}
